...
(
    ...
    FOREIGN KEY (user_level) REFERENCES user_level (ID) ON DELETE
        SET
        NULL
);
...
(
    ...
    FOREIGN KEY (task_type) REFERENCES task_type (ID) ON DELETE CASCADE
);
...
(
    ...
    note             NVARCHAR(100),
    FOREIGN KEY (apartment_ID) REFERENCES apartments (ID) ON DELETE CASCADE,
    FOREIGN KEY (UserThatUploadID) REFERENCES users (ID) ON DELETE CASCADE,
    FOREIGN KEY (expense_type) REFERENCES expense_type (ID) ON DELETE CASCADE
);
...
INSERT INTO task_type (ID, task_type)
VALUES (1, 'general task'),
       (2, 'cleaning'),
       (3, 'dish'),
       (4, 'laundry'),
       (5, 'hang laundry'),
